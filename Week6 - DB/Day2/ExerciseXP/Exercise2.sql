...
SELECT film_id, title, description, length, rental_rate FROM film WHERE title='The Sixth Sense'
...
SELECT * FROM (SELECT *, ROW_NUMBER() OVER (ORDER BY rental_rate ) AS row_num FROM film ) subquery WHERE row_num BETWEEN 11 AND 20
...
SELECT customer.customer_id, (customer.first_name,customer.last_name) AS customer, amount, payment_date, staff.staff_id 
FROM customer INNER JOIN payment ON payment.customer_id = customer.customer_id INNER JOIN staff 
ON payment.staff_id = staff.staff_id ORDER BY staff.staff_id