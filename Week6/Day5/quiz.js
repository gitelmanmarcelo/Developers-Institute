...
//     ```
//     answer: [] because it is the result of what has been deleted. Since 0 elements were deleted the result is empty.
...
//     answer: Yes - we can define it. It is called function expression
//     we invoke it like x(4,5), will return 20, thats 4*5
...
//     answer: Yes. After we have called this x() function we can use the a variable anywhere because it is global
...
//     answer: Yes. Its a callback function. It returns "Hello, JavaScript!"
...
//       the goal of this parameter `num2=10` is to attribute the value of 10 to the num2 variable if it
//       is not provided by the caller


// 10. How would you call the `foo` function in the console.log, in order to 
// display what the `bar` function returns ? Explain the process and concept behind it
...
//      answer:
//      console.log(foo()())
//      It is a closure, where a function returns another functions as its result. So you must invoke the result,
//      using the () to run the function
...
// answer: files answer.html and answer.js
...
//          answer: passed = false, because 42 is smaller than 50 and every item should be greater
...
//     answer: [50,88] because they are greater than 10
...
//     answer: input.forEach( (el, index) => { input[index] **= 2} )
...
//      answer: total = secondInput.filter(el => el > 0).reduce( (acc,el) => acc+el)
...
//      answer: initials = input.split(' ').map(el => el.slice(0,1)).join('')
...
//     answer: they are passed by reference. What is passed is the memory address where the object is, 
//     so when we change the object inside a function we are changing the original object
//     In the example, the tshirt color was blue when passed to the function as myshirt. 
//     When we changed the color of myshirt we are changing the color of the object that is saved in 
//     the address of myshirt which is the same position of tshirt - so we changed the tshirt as well.
//     the results will be: blue, red and red


// 21. How would you change the code above, so that when you modify the key `color` from the parameter `myshirt`, 
// it won't change the global variable `tshirt` ?

//   answer: using ... we create a new object in the function
...
//     answer: const {clubs: {france, spain} } = football;
...
//     answer: The output will be "SUCCESS!".
//     The first promise returns 'Success!'. The second promise returns "SUCCESS!" and the third promise console.log it.
//     The promise will be fullfilled at the end of the process
...
// What will be displayed to the console when calling the `test()` function? 
// Explain the process
...
// answer: the result = await promise will wait until the promise is fullfilled which will occur after
// 1000ms with a succeded returned value of 'done!'
// after that it will run console.log(result); resulting in displaying :
// 'done!'
...
//  answer: in the answer.html and answer.js files
...
class Book {
    ...
    constructor(title, author, pages, creation_date, times_read,thrown_away) {
        this.title = title;
        this.author = author;
        this.pages = pages;
        this.creation_date = creation_date;
        this.times_read = times_read;
        this.thrown_away = thrown_away;
    }
...
class Manual extends Book {
    constructor(title, author, pages, creation_date, times_read,thrown_away) {
        super(title, author, pages, creation_date, times_read,thrown_away);
    }
    ...
    shouldThrow() {
        if (Number(Date().slice(11,15)) - this.creation_date > 5)
        this.thrown_away = true;
    }
}
...
class Novel extends Book {
    ...
    shouldThrow() {
        if (this.times_read > 100)
            this.thrown_away = true;
    }

}

const manual1 = new Manual('Top Secret Shuttle Building Manual','Redacted',1147,2013,1,false);

const novel1 = new Novel('Pride and Prejudice','Jane Austen',432,1813,32,false);
